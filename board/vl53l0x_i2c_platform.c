...
int32_t VL53L0X_write_multi(uint8_t address, uint8_t index, uint8_t *pdata, int32_t count)
{
    ...
    i2c_write_blocking(i2c1, address, index, 1, false);
    i2c_write_blocking(i2c1, address, pdata, count, false);
    if(status != STATUS_OK)
    {
        RANGING_SENSOR_COMMS_Get_Error_Text(debug_string);
    }
...
int32_t VL53L0X_read_multi(uint8_t address, uint8_t index, uint8_t *pdata, int32_t count)
{
    ...
    // Funcao que faz uma leitura bloqueante na linha I2C1, no endereco 'address'.
    // O primeiro parametro 'address' eh o endereco do dispositivo I2C que sera lido.
    // O segundo parametro '0' eh o numero de pagina do dispositivo, que sera desconsiderado
    // pois o dispositivo VL53L0X nao suporta pagina.
    // O terceiro parametro 'index' eh o endereco do registro que sera lido.
    // O quarto parametro 'pdata' eh um ponteiro para um vetor de bytes que sera preenchido
    // com o conteudo do registro lido.
    // O quinto parametro 'count' eh o numero de bytes que sera lido do registro.
    //status = RANGING_SENSOR_COMMS_Read_V2W8(address, 0, index, pdata, count);
    // Faz uma escrita bloqueante na linha I2C1, no endereco 'address', com 'count' bytes de dados em 'pdata'.
    // O parametro 'false' indica que a funcao NAO ira gerar um pulso de clock de reset
    // antes de iniciar a transferencia.
    i2c_write_blocking(i2c1, address, index, 1, false);
    i2c_read_blocking(i2c1, address, pdata, count, false);
    if(status != STATUS_OK)
    {
        RANGING_SENSOR_COMMS_Get_Error_Text(debug_string);
    }
